...
class GameMainPage extends Component {
  state = {
    clickedId: tabsList[0].tabId,
    score: 0,
    url: imagesList[0].imageUrl,
    displayImgList: [imagesList[0].id],
    gameEnd: false,
    seconds: 60,
  }
  ...
  clickedImg = id => {
    const {displayImgList, seconds} = this.state
    console.log(id)
    if (displayImgList.includes(id) && seconds > 0) {
      const randomval = Math.floor(Math.random() * imagesList.length)
      console.log(imagesList[randomval].id)
      this.setState(prevState => ({
        score: prevState.score + 1,
        url: imagesList[randomval].thumbnailUrl,
        displayImgList: [imagesList[randomval].id],
      }))
    } else if (!displayImgList.includes(id) || seconds <= 0) {
      const randomval = 0
      clearInterval(this.timerId)
      this.setState(prevState => ({
        url: imagesList[randomval].thumbnailUrl,
  ...
  pressed = () => {
    this.setState({gameEnd: false, score: 0, seconds: 60})
  }
  ...
  render() {
    const {clickedId, score, url, displayImgList, gameEnd, seconds} = this.state
    const filteredList = imagesList.filter(
      eachImg => eachImg.category === clickedId,
    )
      ...
      <div className="bg-container">
        <nav className="nav-bar">
          <img
            ...
            className="logo"
          />
          <ul className="timer-score">
            <NavItems score={score} seconds={seconds} />
          </ul>
        </nav>
          ...
          {gameEnd ? (
            ...
            <div className="main-page">
              <div className="tumbnail">
                <img src={url} alt="match" className="tumbnail-img" />
              </div>
              <ul className="tabs-con">
                {tabsList.map(eachTab => (
                  <TabItems
                    ...
                    extraVal={eachTab.tabId === clickedId}
                  />
                ))}
              </ul>
